...
class td_video_support{
	...
	static function render_video($videoUrl) {
        ...
        /* Categories and hostname */
        $hostname = parse_url(get_site_url())["host"];
        $categoryCodes = self::get_breadcrumbs_array();
        if (!empty($categoryCodes)) {
            $categoryCodes = '&categoryCode='. self::encode_URI_component($categoryCodes);
        }

		switch (self::detect_video_service($videoUrl)) {
            case 'videojs':
                // \sp3\includes\Player\VideoJs.php
                require_once get_theme_file_path("includes/Player/VideoJs.php");
                $buffy = Player_VideoJs::render($videoUrl, $hostname, $categoryCodes);
                break;
		    case 'youtube':
				$buffy .= '
        ...
        // write out categoryCode if defined
        $buffy .= '<script>window.td_categoryCodes = "'.$categoryCodes.'";</script>';
		return $buffy;
	}
     ...
     */
    private static function detect_video_service($videoUrl, $post_id = null) {
        if (is_null($post_id)) {
            global $post;
            $post_id = $post->ID;
        }
        //read from post meta
        $videoServiceArray = get_post_meta($post_id, 'video_service', true);
        if(!empty($videoServiceArray) && $videoServiceArray['video_service'] != 'auto'){
            return $videoServiceArray['video_service'];
        }

        //if not found in meta data, detect from url
    ...
    /**
     * This function is called from this class and AdBoxRender class.
     * @return string breadcrumb list
     */
    public static function get_breadcrumbs_array() {

        $primary_category_id = td_global::get_primary_category_id();
        $cat_obj = get_category($primary_category_id);

        if (is_wp_error( $cat_obj ) || empty($cat_obj->name)) {
            return '';
        }

        $cat_list = $cat_obj->name;

        while(!empty($cat_obj->parent) && $cat_obj->parent != 0 && !is_wp_error( $cat_obj )) {
            $cat_obj = get_category($cat_obj->parent);
            $cat_list = $cat_obj->name . "," . $cat_list;
        }
